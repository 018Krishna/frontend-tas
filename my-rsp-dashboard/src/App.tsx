...
function monthShortFromLabel(label: string): string {
  ...
  return mapping[key] || 
  (m.charAt(0).toUpperCase() + m.slice(1,3).toLowerCase())
}
...
function findPriceIndex(header: string[]) 
{
  for (let i = 0; i < header.length; i++)
    ...
    if (h.includes('retail') || h.includes('rsp') || h.includes('selling price') || h.includes('price')) {
      return i
    }
  }
...
function App() 
{
  ...
  useEffect(() =>
   ...
   if (rows.length < 2) { setError('CSV seems empty or malformed'); setLoading(false); return }
       ...
       {
        setError(String(err.message || err)); setLoading(false)
      }
    }
    load()
  }, [])
  ...
  return (
    <div className="container"
      ...
      style={{ marginTop: 20, fontSize: 12, color:'#6b7280' }}>
